...
public class CategoryProductQueryService {
    ...
    public Page<CategoryProduct> findAllByProductId(CategoryProductQuery query, PageRequest pageRequest, ERole role) {
        Criteria criteria = new Criteria();

        if (query.productId() != null) {
            criteria.and("productId").is(new ObjectId(query.productId()));
        }

        if (query.search() != null && !query.search().isBlank()) {
            criteria.orOperator(
                    Criteria.where("user.firstName").regex(".*" + query.search() + ".*", "i"),
                    Criteria.where("user.lastName").regex(".*" + query.search() + ".*", "i"),
                    Criteria.where("user.email").regex(".*" + query.search() + ".*", "i"),
                    Criteria.where("category.name").regex(".*" + query.search() + ".*", "i"),
                    Criteria.where("product.name").regex(".*" + query.search() + ".*", "i"),
                    Criteria.where("product.description").regex(".*" + query.search() + ".*", "i")
            );
        }

        if (query.fromDate() != null && query.toDate() != null){
            LocalDateTime newFromDate = LocalDateTime.parse(query.fromDate() + "T00:00:00");
            LocalDateTime newToDate = LocalDateTime.parse(query.toDate() + "T23:59:59");
            criteria.and("createdAt").gte(newFromDate).lte(newToDate);
        }
